...
class Log : public MaNGOS::Singleton<Log>
{
    ...
    public:
        ...
        */
         ...
         *        campaign's Stage 2 world-login swap) was the only caller of
         *        this and of outWorldPacketDump()/outWorldPacketDumpRedacted()
         *        above, and it is gone. The config flag still parses and this
         *        API still compiles, but nothing invokes it, so setting
         *        PacketLoggingEnabled=1 today produces no dump file. Re-homing
         *        it into proto::ClientConnection::onData()/SendPacket() (the
         *        new choke points for every inbound/outbound WorldPacket) is a
         *        deliberate follow-up, not folded into this change so a
         *        logging re-home does not ride along with an unrelated commit.
         * @return bool
         */
