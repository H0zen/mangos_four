...
// LiquidType.dbc -> { id : (category, spellId, materialId) }.
//
// In 5.4.8 the `SoundBank` column (called `Type` before Cataclysm) is authoritative and
// reads 0 = water, 1 = ocean, 2 = magma, 3 = slime. That is NOT the 2.4.3 encoding
// (0 = magma, 2 = slime, 3 = water, ocean indistinguishable), which is why the 2.4.3
// extractors classify rows below 21 by id arithmetic instead. Here the column answers
// directly.
//
// `materialId` is the second half of the MH2O vertex-format chain -- see
// LiquidObjectStore.
...
namespace world
{
    ...
    {
        uint32_t type = 0;        ///< the SoundBank column; see LiquidDbcType
        uint32_t spellId = 0;     ///< aura applied while in this liquid
        uint32_t materialId = 0;  ///< LiquidMaterial.dbc row, column 14
    };
    ...
    public:
        bool LoadFromDbc(world::terrain::IMpqArchive& archive)
        {
            ...
            for (uint32_t r = 0; r < dbc.GetNumRows(); ++r)
            {
                ...
                info.materialId = rec.getUInt(14);
