...
namespace world::nav
{
    namespace
    {
        ...
        bool BakeTile(const MapBake& mb, int gx, int gy, bool& tileError)
        {
            ...
            // These three carry #250's accuracy tuning, restored after an A/B/C on map
            // 870 -- 30k points against the baked floor, binned by slope, since flat
            // ground matches any mesh. Points off by over a yard fall 17.6% -> 3.1% on
            // gentle slope and 40.5% -> 10.9% on steep, for +14% mmap size. #250 also
            // raised walkableClimb and added rcMedianFilterWalkableArea; a third run
            // showed those contribute nothing here, so they stay out until something
            // measures what they DO change, which is connectivity.
            cfg.maxSimplificationError = 1.8f;
            cfg.detailSampleDist = cfg.cs * 16.0f;
