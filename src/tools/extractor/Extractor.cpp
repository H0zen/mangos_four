...
// The offline baker. One pass over a 5.4.8 client's MPQs, producing the caches
...
#include "stores/LiquidObjectStore.hpp"
...
"             Reads baked gomodels only -- no client needed. Run it again\n"
"             whenever the `transports` table gains a row.\n"
...
int main(int argc, char** argv)
{
    ...
    g_console.Start(opt.src, opt.dest, "5.4.8");
    ...
    if (!opt.dbc && !opt.tiles && !opt.goModels)
    {
        ...
        const bool ok = vesselsOk && BakeNav(opt, tileDir, vesselMaps);
    ...
    const int opened = mpq.OpenPatchedClientData(
        opt.src, ClientArchives548(), ClientLocaleArchives548(),
        ClientUpdates548(), ClientLocaleUpdates548(), opt.locale);
    if (!opened)
    {
    ...
    if (opt.dbc && opt.allLocales)
    {
        for (const std::string& loc : FindLocales(opt.src))
        {
            ...
            if (!other.OpenPatchedClientData(opt.src, ClientArchives548(),
                                       ClientLocaleArchives548(),
                                       ClientUpdates548(),
                                       ClientLocaleUpdates548(), loc))
            {
    ...
    // LiquidType -> LiquidMaterial. Without it a depth-only instance is read as a
    // heightmap, which bakes water surfaces near 1e11 -- a number no later check
    // rejects. A warning rather than a hard stop: the fallback still handles every
    // instance that names a plain vertex format.
    world::LiquidObjectStore liquidObjects;
    if (!liquidObjects.LoadFromDbc(mpq))
    {
        g_console.Warn("LiquidObject.dbc or LiquidMaterial.dbc could not be read -- "
                       "MH2O vertex formats fall back to the pre-5.x rule");
    }
