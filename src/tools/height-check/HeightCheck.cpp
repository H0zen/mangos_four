...
namespace
{
    ...
    // The three ways a probe corpus lies, kept apart from the score rather than averaged
    // into it. A client-recorded probe settles a SWIMMING body at the liquid surface and
    // an INDOOR one on a WMO floor, so both answer a different question from "where is
    // the terrain"; and a probe recorded before the client finished correcting the
    // position simply hands back the Z it was sent, which agrees with anything.
    struct Confounders
    {
        uint32_t wet = 0;        ///< a liquid surface stands at or above the probe
        uint32_t onStatic = 0;   ///< the winning floor is baked model geometry, not ADT
        uint32_t unmoved = 0;    ///< expected == z exactly: nothing settled
        double wetSumAbs = 0.0;
        double drySumAbs = 0.0;
        uint32_t wetScored = 0;
        uint32_t dryScored = 0;
        uint32_t dryWithin2 = 0;
    };
...
int main(int argc, char** argv)
{
    ...
    for (const Probe& p : probes)
    {
        ...
        // Which surface won, so an indoor probe is not read as a terrain miss.
        world::terrain::SurfaceKind winner = world::terrain::SurfaceKind::Terrain;
        if (floor)
        {
            for (const world::terrain::Surface& sf : column.Surfaces())
            {
                if (sf.Solid() && sf.z == *floor)
                {
                    winner = sf.kind;
                    break;
                }
        ...
        {
            ++conf.wet;
            conf.wetSumAbs += delta;
            ++conf.wetScored;
        }
        else
        {
            conf.drySumAbs += delta;
            ++conf.dryScored;
            if (delta <= 2.0f)
            {
                ++conf.dryWithin2;
        ...
        if (winner == world::terrain::SurfaceKind::Static)
        {
            ++conf.onStatic;
        }
        if (p.expected == p.z)
    ...
    // A corpus recorded from a running client answers a different question for a body
    // that is swimming or standing indoors, and answers NOTHING for one recorded before
    // the client finished settling. All three inflate a score rather than depress it,
    // which is why they are reported whether or not anyone thought to look.
    if (scored)
    {
        std::printf("\nconfounders\n");
        std::printf("  over liquid   : %u of %u (%.1f%%), mean|d| %.2f -- a swimming body"
                    " settles at the SURFACE, not the floor\n",
                    conf.wet, scored, 100.0 * double(conf.wet) / double(scored),
                    conf.wetScored ? conf.wetSumAbs / conf.wetScored : 0.0);
        std::printf("  on model floor: %u of %u (%.1f%%) -- WMO interior, not heightmap"
                    " terrain; a different half of the engine\n",
                    conf.onStatic, scored,
                    100.0 * double(conf.onStatic) / double(scored));
        // Read this ONLY against how the corpus was made, and it is diagnostic in just
        // one of the three cases. Both a database corpus and a corpus of positions the
        // client REPORTED have expected == z by construction, so 100% there is correct
        // and says nothing. It means something only when the probe was SENT a z and the
        // client's correction was read back: a row that matches was recorded before the
        // client finished settling, and agrees with any terrain model at all.
        std::printf("  expected == z : %u of %u (%.1f%%) -- by construction unless the"
                    " probe was sent a z and the client's correction read back\n",
                    conf.unmoved, total.probes,
                    100.0 * double(conf.unmoved) / double(total.probes));

        if (conf.dryScored)
        {
            std::printf("\ndry ground only: %.2f%% within 2 yards over %u probes,"
                        " mean|d| %.2f\n",
                        100.0 * double(conf.dryWithin2) / double(conf.dryScored),
                        conf.dryScored, conf.drySumAbs / conf.dryScored);
        }
    }
