...
{
    // The 18414 body is a fixed 24 bytes in a different order, and every field is
    // always present. See MopCompactPackets::BuildSendMailResult; the body that
    // stood here was 12, 16 or 20 bytes depending on the outcome.
    WorldPacket data(SMSG_SEND_MAIL_RESULT, 24);
    MopCompactPackets::BuildSendMailResult(data, mailId, equipError,
        uint32(mailError), uint32(mailAction), item_guid, item_count);
