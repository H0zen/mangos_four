...
void Camera::UpdateVisibilityForOwner()
{
    ...
    // The other side of a vessel's boundary. A deck and the shore it sails past are two maps,
    // and no cell visit of one reaches the other -- so the same notifier is run over the far
    // side as well. ONE elimination follows, over both, which is the whole point: an object that
    // drops out of reach across the boundary is destroyed by the ordinary sweep instead of by a
    // ledger somebody has to remember to keep.
