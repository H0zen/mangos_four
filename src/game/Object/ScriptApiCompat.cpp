...
void WorldObject::GetPosition(WorldLocation& loc) const
{
    loc.mapid = GetMapId();
    loc.coord_x = Where().X();
    loc.coord_y = Where().Y();
    loc.coord_z = Where().Z();
    loc.orientation = Where().Facing();
}
...
{
    return obj && Where().ShareFrame(obj->Where()) && InSamePhase(obj);
}
...
bool WorldObject::IsWithinLOS(float x, float y, float z,
                              world::terrain::ModelIgnoreFlags ignoreFlags) const
{
    return HasLineOfSight(*this, Geometry::Vector3(x, y, z), ignoreFlags);
}

bool WorldObject::IsWithinLOSInMap(WorldObject const* obj,
                                   world::terrain::ModelIgnoreFlags ignoreFlags) const
{
    return obj && HasLineOfSight(*this, *obj, ignoreFlags);
}
