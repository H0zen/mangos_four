...
bool Pet::LoadPetFromDB(Player* owner, uint32 petentry, uint32 petnumber, bool current, int32 slot)
{
    ...
    // Nothing to board. `map` is already the map the owner is on, and aboard that IS the deck:
    // the CreatureCreatePos this pet was built from placed it beside him in the hull's own
    // coordinates, and its create block derives the transport parent from the map it lands on.
