...
void WorldObject::SendMessageToSet(WorldPacket* data, bool /*bToSelf*/) const
{
    ...
    // A boarded unit's audience is EVERYONE ON ITS OWN MAP -- which is the deck, and which is
    // exactly where the passengers are too. That is the whole point of the vessel being a map:
    // the ordinary broadcast already reaches the people standing next to him.
    GetMap()->MessageBroadcast(this, data);

    // THE RELAY, OUTBOUND. The people ashore are on another map and no cell of theirs will ever
    // hold this deckhand, so the same packet goes out again to the watchers the vessel gathered
    // at the top of this tick. Sent immediately: the deck runs INSIDE the tick of the map it
    // sails, on that map's own thread, so there is nothing to wait for and no race to avoid.
    ...
    // THE RELAY, INBOUND, and it is not filtered by anything. Whatever happens on the water a
    // ship is crossing has to reach the people standing on her: they are on another map, no cell
    // of theirs will ever hold the thing that moved, and a passenger who is told nothing watches
    // a harbour full of statues.
    //
    // No distance test. The one object that could measure it is the vessel, whose pose is an
    // estimate we already refuse to trust for anything that decides something.
