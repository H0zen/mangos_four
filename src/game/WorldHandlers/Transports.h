 ...
 *
 * It has three jobs and no fourth:
 *
 *  1. Run the route clock, whose only decision is WHEN the ship changes world map, and hand
 *     the client the path progress it needs. That number and her entry are the whole of what
 *     the client is told; it draws the hull itself out of TransportAnimation.dbc, which the
 *     server does not load. Her create block carries the position (0, 0, 0) and always will.
 ...
 * is told about them -- belongs to the map and is asked of the map (`AsMap()`), because a ship
 * is not a container of people, it is a place.
 *
 * The waypoint pose is not a position and is never treated as one. Nothing is redrawn from it,
 * nothing is repositioned by it, and nothing is ever composed with it. It exists to name the
 * WORLD GRID to look in when the relay goes looking for what is ashore.
 */
class Transport : public GameObject
{
    public:
        ...
        /// Take the vessel out of the world it sails. Called while the maps are still alive --
        /// nothing else does it, because a vessel is in no cell and no grid unload reaches it.
        ...
        /**
         * @brief Resolve, once at start-up, the Map.dbc id of a vessel's hull.
         *
         * Blizzard ships a row per vessel whose Directory is "Transport<goEntry>" and no
         * terrain for it -- the hull only ever existed as a game object model, which the baker
         * turns into that map's terrain. 5.4.8's Map.dbc format string does load Directory, so
         * that row is matched here rather than guessed. Vessels Blizzard shipped no row for get
         * an id minted instead, and a Map.dbc entry injected to carry it. Nothing on the wire
         * ever carries this number.
         */
        ...
        /// Pin every world grid the route passes through, at start-up and for good. The vessel
        /// is an active object in those grids: what the relay finds ashore is whatever those
        /// cells hold, and a cell that had expired holds nothing.
        void PinRouteGrids();
        ...
        /// How far along her route she is, in milliseconds of path time. The one number the
        /// client needs to place the hull: it interpolates her from this and the path, and we
        /// estimate her from the same value, so both sides agree.
        uint32 GetPathProgress() const { return m_timer; }

        /// Half the longest gap between consecutive waypoints: how far the estimate can be off,
        /// since it snaps between nodes and never interpolates. Every proximity question about
        /// this vessel is widened by it.
    ...
    private:
        ...
        /// The world map she is on her way to, and nothing yet done about it. The point is the
        /// TAXI NODE's own, copied out of the route the moment it was read: what the passengers
        /// are sent to must be the number the client's own path is built from, not a pose we
        /// recomputed on the way past it.
        uint32 m_crossingTo;
        float m_crossingX;
        float m_crossingY;
        float m_crossingZ;
