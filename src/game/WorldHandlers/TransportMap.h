 ...
 *
 * The hull's baked geometry is this map's terrain, so height, collision and routing aboard are
 * answered by the ordinary engines; model space is this map's space, so a position aboard is
 * composed with nothing and transformed by nothing. What is left over is what a continent does
 * not have, and it all lives here rather than on the game object:
 *
 ...
 *
 * The game object keeps the route and its own pose, because someone has to know where to look
 * for those observers. It keeps nothing else.
 */
class TransportMap : public Map
{
    public:
        TransportMap(uint32 id, time_t expiry, Transport* vessel)
            : Map(id, expiry, 0, REGULAR_DIFFICULTY), m_vessel(vessel), m_commissioned(false),
              m_hullRadius(0.0f) {}
        ~TransportMap() {}
         ...
         *
         * Not a map in the scheduler's rotation: it has no independent existence, so it runs on
         * the thread of the map the vessel sails, after that map has finished walking its own
         * containers. That is what makes every packet sent from here sendable at once, with no
         * queue and no tick of latency.
         */
         ...
         *        borrow any of it.
         *
         * His client has never heard of this map and never will: it was told the world map the
         * ship sails, and it is still rendering it. So there is no new world to introduce --
         * only the ship herself, whom no sweep of his can ever reach because she stands on that
         ...
         * relay -- but it takes nobody aboard: Map::Add cannot load a grid with no terrain, so
         * whoever stepped on would be removed from the world he was in and added to nothing.
         */
        bool Commission();
        ...
        /// The surface under a point: the highest one at or below it. Nothing when there is
        /// none -- which is a REFUSAL, not an invitation to fall back to the water the ship is
        /// crossing.
         ...
         *
         * The requested bearing is tried first and then swept around the master, because a ship
         * is small and cluttered and the one spot asked for is very often out over the rail.
         * Nothing when `master` is not aboard.
         */
        ...
        std::optional<Position> PositionOf(WorldObject const& obj) const;

        // --- Who is aboard ---------------------------------------------------------

        /// Bring a player onto this map. Only once his client is already in the world and has
        /// been sent the vessel -- it must never be told this map id, having no terrain for it.
        void Embark(Player* passenger);

        /// Put him down on the map the ship sails, at the point the CALLER names. Whoever ends
        /// the voyage owns the destination.
        void Disembark(Player* passenger, float x, float y, float z, float o);

        // --- The seam ----------------------------------------------------------------
        //
        // The vessel announces WHEN she crosses and nothing more; she knows nothing about the
        // people on her. What crossing MEANS for them is decided here, on both sides.
         ...
         * transfer packet they are sent names the map they are leaving.
         */
        ...
        /**
         * @brief Index a creature that has arrived. An INDEX, not a registration: nothing here
         *        decides whether it is aboard, its map already did.
         *
         * Called from Creature::AddToWorld/RemoveFromWorld -- the one place both the grid
         * loader and Map::Add pass through. A creature with an owner is filed as a MINION and
         * not as crew: crew are exempt from the visibility sweep, and a minion that is exempt
         * is a minion no client ever destroys.
         */
        void EnlistCrew(Creature* crew);
        void DelistCrew(Creature* crew);
        bool HasCrew() const { return !m_crew.empty(); }

        /// Drop the indices at shutdown. The creatures are this map's to destroy, like any
        /// other map's.
        void ReleaseCrew() { m_crew.clear(); m_minions.clear(); }

        // --- What the shore is told ------------------------------------------------
        //
        // A vessel is not seen the way a mob is. A mob lives in a cell and the visibility sweep
        // polls distance to it every tick; a ship's pose is a waypoint estimate that disagrees
        // with the curve the client draws, so any distance test flickers. So possession is MAP
        // MEMBERSHIP and it is edge-triggered: you have a vessel, and everyone aboard her, for
        // as long as you share the map she sails.
         ...
         * drops out across the boundary is destroyed by the ordinary sweep, with no ledger and
         * no special case anywhere.
         */
        static void CollectRelaySources(WorldObject const* viewer, float visibility,
                                        std::vector<RelaySource>& out);

        /// One newly-arrived crew member, announced to everyone already watching.
    ...
    private:
         ...
         * ways a minion comes to be standing here -- its master walks aboard, it is summoned at
         * sea, its master logs in mid-voyage, a totem is dropped on the forecastle -- and only
         * one way to be sure we caught them all.
         */
        void UpdateMinions();
        ...
        /// Owned creatures standing here: pets, guardians, totems. Ordinary objects to the
        /// visibility sweep, unlike m_crew -- this index exists only for the reconciler.
        std::vector<Creature*> m_minions;
};
