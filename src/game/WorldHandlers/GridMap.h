...
// The ONE value that means "no height". There used to be a second,
// INVALID_HEIGHT_VALUE, returned where this one was compared against -- so a caller
// that checked the wrong constant accepted -200000 as a real height and never said so.
// Absence is std::optional everywhere inside the core; this survives only for the
// scripting modules, which cannot be changed from this repository.
#define INVALID_HEIGHT       -100000.0f

// A floor at most this far above the point still counts as the one it stands on. Small
// on purpose: a wider tolerance answers someone on a floor with the roof over their head.
#define FLOOR_SEARCH_UP            2.0f

// How far the sweep starts ABOVE the point anyway, so a position buried in geometry -- a
// spawn a yard into a hillside -- still finds the surface it would stand on. Buried
// deeper than this is a broken coordinate, and no floor is the honest answer.
