...
void Spell::SetTargetMap(SpellEffectIndex effIndex, uint32 targetMode, UnitList& targetUnitMap)
{
    ...
    switch (targetMode)
    {
        ...
        case TARGET_TOTEM_FIRE:
        {
            ...
            const Geometry::Vector3 spot = PointNear(*m_caster, radius, angle);
            x = spot.x;
            y = spot.y;
        ...
        case TARGET_RANDOM_UNIT_CHAIN_IN_AREA:
        {
            ...
            while (t && next != tempTargetUnitMap.end())
            {
                ...
                if (!DisableMgr::IsDisabledFor(DISABLE_TYPE_SPELL, m_spellInfo->ID, NULL, SPELL_ATTR_EX2_IGNORE_LOS) && !HasLineOfSight(*prev, **next, world::terrain::ModelIgnoreFlags::M2))
                {
                    ++next;
                    continue;
                }

                if (!HasLineOfSight(*prev, **next, world::terrain::ModelIgnoreFlags::M2)
        ...
        case TARGET_DYNAMIC_OBJECT_RIGHT_SIDE:
        {
            if (!(m_targets.m_targetMask & TARGET_FLAG_DEST_LOCATION))
            {
                ...
                float x, y;
                const Geometry::Vector3 spot = PointNear(*m_caster, radius + m_caster->Where().Extent(), angle);
                x = spot.x;
                y = spot.y;
        ...
        case TARGET_POINT_AT_SW:
        {
            if (!(m_targets.m_targetMask & TARGET_FLAG_DEST_LOCATION))
            {
                ...
                float x, y;
                const Geometry::Vector3 spot = PointNear(*currentTarget, radius + currentTarget->Where().Extent(), angle);
                x = spot.x;
                y = spot.y;
