...
/**
 * @file GridMap.cpp
 * @brief The game's half of the terrain: DBC meaning over baked geometry.
 *
 * world::terrain::FusedTerrain answers in world coordinates and in the identifiers
 * baked into a tile. TerrainInfo is what turns those into MaNGOS's world -- a WMO
 * group into a WMOAreaTable entry, a LiquidType row into the flags the client
 * expects, an AreaTable id into the area bit the rest of the server passes around.
 *
 * @see GridMap.h for the class
...
TerrainManager::TerrainManager() : m_lock()
{
}
 ...
 *
 * Mists nests areas several levels deep, so the zone is the ROOT of the parent
 * chain, not the immediate parent -- see MopTerrain::ResolveRootAreaId, which also
 * refuses to loop on a cyclic chain.
 *
 * @param areaflag The area explore flag.
 * @param map_id The map id.
