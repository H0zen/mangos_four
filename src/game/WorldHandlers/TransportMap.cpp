...
namespace
{
    ...
    const float HULL_SEARCH_UP = 3.0f;
    const float HULL_SEARCH_DOWN = 6.0f;

    /// Chest height for the obstruction probe, so the plating a unit stands ON is not itself
    /// read as the thing blocking it.
    const float HULL_PROBE_HEIGHT = 1.0f;

    /// Bearings tried when sweeping for a free spot around a master.
    const int HULL_SPOT_BEARINGS = 8;

    /// A deck is one place. Nothing on it is phased away from anything else on it.
    const uint32 DECK_PHASE = PHASEMASK_NORMAL;
     ...
     * exactly right, and one dropped on the forecastle sails with the ship.
     */
    bool IsPlanted(Creature const* minion)
    {
     ...
     *
     * See CREATURE_TYPEFLAGS_TRANSPORT_FORBIDDEN. Cached per ENTRY in the client's creature
     * query cache, so it cannot be papered over per spawn on the wire: the creature simply
     * does not sail.
     */
    ...
    /// Drop a minion from one client, both halves: the packet AND the server's record that the
    /// client holds it. Either one alone leaves the two disagreeing.
    ...
    void DrawMinionTo(Unit* minion, Unit* master, Map* dest)
    {
        ...
        if (minion->GetTypeId() != TYPEID_UNIT || !minion->IsInWorld() || !minion->IsAlive())
        {
            return;
        }
        ...
        DEBUG_FILTER_LOG(LOG_FILTER_DECK_MINIONS,
                         "DrawMinionTo: %s map=%u -> map=%u%s beside %s",
                         c->GetGuidStr().c_str(), c->GetMapId(), dest->GetId(),
                         dest->AsTransport() ? "[deck]" : "", master->GetGuidStr().c_str());

        float x, y, z;
        master->GetClosePoint(x, y, z, c->GetObjectBoundingRadius(), PET_FOLLOW_DIST,
                              PET_FOLLOW_ANGLE);

        // Whatever leg it was on was planned in the map it is leaving; none of it survives.
        c->StopMoving();

        // TELL EVERY CLIENT TO DROP IT FIRST, and MAKE THE SERVER AGREE. Without this the ones
        // already holding it get only a heartbeat at the far side and interpolate the
        // difference -- which a player sees as his pet swimming up through the hull.
        //
        // The packet alone is not enough and that is the whole trap: DestroyForPlayer does not
        // touch m_clientGUIDs, so the server still believes the client holds it, and the
        // visibility pass at the far side takes the already-have branch and sends NO CREATE.
        //
        // And THE MASTER MUST BE IN THE LIST. He is the one client that has to re-create it,
        // and the one this loop cannot reach: by the time a minion is drawn ashore he has
        // already been taken off the deck map it is standing on.
        ...
        c->Relocate(x, y, z, master->GetOrientation());
        dest->Add(c);

        // AND THE MOTION, in the frame it now stands in. MoveFollow and MoveIdle each clear the
        // stack themselves; clearing it here first empties it down to and including the idle
        // generator, and the Clear inside MoveFollow then asserts on a non-empty stack.
    ...
    /// Every minion this master controls, moved to `dest` beside him. The reconciler does the
    /// same sweep once per tick; this is the immediate half, so nothing is ever seen standing
    /// where its master no longer is.
...
bool TransportMap::Commission()
{
    ...
    if (model)
    {
        ...
        // would answer no height, no collision and no route -- silently, and only once she was
        // already at sea. The bounds are walked corner to corner because a hull straddles the
        // map centre and so spans up to four grids.
        uint32 pinned = 0;
        for (float gx = b.lo.x; gx < b.hi.x + SIZE_OF_GRIDS; gx += SIZE_OF_GRIDS)
        {
            for (float gy = b.lo.y; gy < b.hi.y + SIZE_OF_GRIDS; gy += SIZE_OF_GRIDS)
            {
                // Clamped to the hull: stepping a whole grid past it pinned cells the ship does
                // not occupy, half a kilometre off the bow.
    ...
    // whoever steps aboard: Map::Add cannot load a grid that has no terrain, so a passenger is
    // removed from the world he was in and added to nothing.
    if (!ColumnAt(DECK_PHASE, 0.0f, 0.0f, m_hullRadius * 3.0f, -m_hullRadius * 3.0f)
             .HighestSolid())
    {
        sLog.outErrorDb("Transport %u (%s, display %u): map %u has no baked terrain. Re-bake "
                        "the vessel maps (see tools/extractor/vessels.txt); it will carry no "
                        "crew and board no passengers until then.",
                        goinfo->id, goinfo->name, goinfo->displayId, GetId());
        return false;
    }
                                             ...
                                             float searchUp, float searchDown) const
{
    ...
    // levels down.
    return ColumnAt(DECK_PHASE, x, y, z + searchUp, z - searchDown)
...
bool TransportMap::IsBlocked(Geometry::Vector3 const& from, Geometry::Vector3 const& to) const
{
    ...
    return !IsInLineOfSight(from.x, from.y, from.z, to.x, to.y, to.z, DECK_PHASE);
}

std::optional<Position> TransportMap::PositionOf(WorldObject const& obj) const
{
    ...
    if (obj.GetMap() == this)
    {
        return Position(obj.GetPositionX(), obj.GetPositionY(), obj.GetPositionZ(),
                        obj.GetOrientation());
    }
                                                   ...
                                                   float angle) const
{
    ...
    // The requested bearing first, then swept around the master: a ship is small and cluttered,
    // and the one spot the caller asked for is very often out over the rail.
    for (int step = 0; step < HULL_SPOT_BEARINGS; ++step)
    {
        ...
        return Position(x, y, *z, anchor->o);
    }
...
bool TransportMap::Add(Player* passenger, bool introduce)
{
    ...
    passenger->Relocate(aboard->x, aboard->y, aboard->z, aboard->o);

    passenger->GetMapRef().link(this, passenger);
    passenger->SetMap(this);
    passenger->InitializePhaseForMapEntry(0);

    CellPair p = MaNGOS::ComputeCellPair(passenger->GetPositionX(), passenger->GetPositionY());
    Cell cell(p);
    EnsureGridLoadedAtEnter(cell, passenger);
    PromoteEnvelopeNeighboursToFull(cell.GridX(), cell.GridY());
    passenger->AddToWorld();

    // The ship, her crew, then the man standing on her -- in that order, and the order is
    // load-bearing: his own block names her guid. SendInitSelf already builds exactly that when
    // the arriving player has a transport, and it is the ONLY builder of the 18414 self block,
    // so it is reused unchanged rather than half-rewritten here.
    //
...
void TransportMap::Embark(Player* passenger)
{
    ...
    // pet standing at the rail while its master walks off, which is what a player actually sees.
...
void TransportMap::Disembark(Player* passenger, float x, float y, float z, float o)
{
    ...
    // player->GetTransport() on the assumption that our own vessel already reached us through
    // SendInitSelf. Stepping ashore is the one case where that is false: leave the pointer set
    // and the ship he just left is the single vessel never announced to him.
                                      ...
                                      float x, float y, float z, float o)
{
    ...
    // From EVERYONE there, not from a range: possession of a vessel is map membership, and this
    // is the moment membership ends.
    ...
    for (Player* passenger : aboard)
    {
        ...
        // himself never leaves this one -- Player::BoardingMap puts him straight back aboard on
        // the far side. The vessel has NOT moved yet, so the transfer packet still names the map
...
void TransportMap::VesselEnteredWorld(Map* newWorld)
{
    ...
    // The same channel that took her away gives her back: everyone on the new map gains her by
    // membership, here and now, with no distance asked.
...
void TransportMap::EnlistCrew(Creature* crew)
{
    ...
    // every client watching the ship crashing the moment she gets under way -- so it is put off
    // rather than carried, and the log says which one.
    if (!CanRide(crew))
    {
    ...
    // An OWNED creature is somebody's minion, not the ship's. Crew are exempt from the
    // visibility sweep because they ride the vessel's map-membership channel; a minion that was
    // exempt would be a minion no client ever destroys.
    std::vector<Creature*>& roster = crew->GetOwnerGuid() ? m_minions : m_crew;
    if (std::find(roster.begin(), roster.end(), crew) == roster.end())
    {
        roster.push_back(crew);
    }

    // Active, because this map's grids are pinned and no player is required to be on it to keep
    // it awake -- the observers are ashore, watching through the vessel.
...
void TransportMap::DelistCrew(Creature* crew)
{
    ...
    m_minions.erase(std::remove(m_minions.begin(), m_minions.end(), crew), m_minions.end());
}

void TransportMap::UpdateMinions()
{
    // A MINION FOLLOWS ITS MASTER ACROSS A MAP BOUNDARY. That is the whole of it, and it is the
    // same move a pet makes following its master through a portal.
    for (PlayerList::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
    {
    ...
    // And the reverse: anything aboard whose master has left. Read from m_minions, never from
    // m_crew -- a pet is not crew, and the roster it is looked for in has to be the one it is
    // actually filed in, or every pet whose master walked ashore stands on the deck for good.
    std::vector<Creature*> stranded;

    for (Creature* aboard : m_minions)
    {
...
void TransportMap::SendCrewMemberCreate(Creature* crew)
{
    ...
    // CREW ONLY. A minion is an ordinary object to the visibility sweep, so it must arrive
    // through that sweep and be stamped into the observer's client set; pushed unconditionally
    // it would be a create nobody ever destroys. Refused here rather than at the call site, so
    // no caller can get it wrong.
    if (crew->GetOwnerGuid())
    {
        return;
    }
    ...
    for (PlayerList::const_iterator itr = everyone.begin(); itr != everyone.end(); ++itr)
    {
        ...
        UpdateData data(observer->GetClientMapId());
        crew->BuildCreateUpdateBlockForPlayer(&data, observer);
        if (!data.HasData())
        {
            continue;
...
void TransportMap::AnnounceVessel(Transport* vessel, Player* observer)
{
    ...
    // The hull AND everyone on her. The observer's own visibility sweep would find the crew too,
    // but only when HE moves -- and a man standing on a pier watching a ship come in does not
    // move. Leaving it to the sweep gave him an empty deck until he stepped aboard.
    UpdateData data(observer->GetClientMapId());
                                       ...
                                       std::vector<RelaySource>& out)
{
    ...
    // Aboard: his own pass walks the ship, and the shore is reached from her estimate. That pose
    // is a lie by up to NodeSlack, which is exactly why it is added.
    if (TransportMap const* hull = viewer->FindMap()->AsTransport())
    {
        Transport* vessel = hull->Vessel();
        Map* sailed = vessel && !vessel->IsCrossing() ? vessel->GetMap() : NULL;
        if (sailed)
        {
            out.push_back({sailed, vessel->GetPositionX(), vessel->GetPositionY(),
    ...
    for (Transport* vessel : vessels->second)
    {
        ...
        if (!vessel->IsWithinDist2d(viewer->GetPositionX(), viewer->GetPositionY(), reach))
        {
            continue;
        }

        // The whole ship, swept from her origin. Her space is small and a crew is a few dozen,
        // so a radius that certainly covers the hull is cheaper than being exact.
...
void TransportMap::GatherObservers()
{
    ...
    // FROM THE GRID, around the estimate. That approximate pose is what the estimate is FOR: it
    // names the cells to look in. Sweeping the whole map's player list instead would answer the
    // same question by reading every player on a continent.
    struct WatchingFromShore
    {
        ...
        bool operator()(Player* watcher) const
        {
            return watcher->IsInWorld() &&
                   focus->IsWithinDist2d(watcher->GetPositionX(), watcher->GetPositionY(), range);
        }
    };
    ...
    // For BROADCAST only -- a spline, an emote, a spell go, anything said aboard while nobody's
    // visibility pass happens to be running. Visibility itself is decided by each viewer's own
    // sweep, which reaches across through CollectRelaySources.
