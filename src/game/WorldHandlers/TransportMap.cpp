...
bool TransportMap::Add(Player* passenger)
{
    ...
    // 5.4.8 has no MOVEFLAG_ONTRANSPORT -- being aboard is carried by the transport data
    // itself. What this line is FOR is the reset TeleportTo would have done: a stale run
    // flag leaves him running on the spot for anyone ashore until he moves again.
    ...
    // THE MAP ID ON THE WIRE IS THE WATER SHE SAILS, never this map's. SMSG_UPDATE_OBJECT
    // carries it as a uint16, so a minted deck id would not even survive the field
    // (1020808 & 0xFFFF = 37768) -- but the real reason is simpler: his client has never
    // heard of this map and is still rendering the one the ship is crossing.
    UpdateData data(uint16(passenger->GetClientMapId()));
...
void TransportMap::SendCrewMemberCreate(Creature* crew)
{
    ...
    for (PlayerList::const_iterator itr = everyone.begin(); itr != everyone.end(); ++itr)
    {
        ...
        UpdateData data(uint16(observer->GetClientMapId()));
        crew->BuildCreateUpdateBlockForPlayer(&data, observer);
