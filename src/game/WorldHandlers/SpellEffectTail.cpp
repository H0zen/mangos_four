...
void Spell::EffectTransmitted(SpellEffectEntry const* effect)
{
    ...
    else
    {
        ...
        if (goinfo->type == GAMEOBJECT_TYPE_FISHINGNODE)
        {
            ...
            const Geometry::Vector3 spot = PointNear(*m_caster, dis + m_caster->Where().Extent(), m_caster->Where().Facing() + angle_offset);
            fx = spot.x;
            fy = spot.y;
...
void Spell::EffectBind(SpellEffectEntry const* effect)
{
    ...
    {
        loc = WorldLocation(player->GetMapId(),
                            player->Where().X(), player->Where().Y(),
                            player->Where().Z(), player->Where().Facing());
        if (!area_id)
        {
